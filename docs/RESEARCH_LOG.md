# open-hearts — full research log

> The complete phase-by-phase research chronicle: every experiment, pre-registration, result table, chart, null, and limitation, in the order they happened. The front-page [README](../README.md) is the short version; this file is the evidence. Numbers here are the record — nothing was moved to this file to soften it.
...
|---|---|---|
| **vs. xinxin** (Sturtevant's reference Hearts bot, at its published 2,000-run/50-world/C=0.4 configuration; no-pass/no-moon variant) — paired diff **−0.776 [−0.910, −0.642] in our favor**, Wilcoxon z=−11.3, at ~9× less time per decision | **6.11** (xinxin: 6.89) | 1,000 deals × 14 mixed seating configurations = 14,000 games |
...
![Guessing accuracy by trick](guessing.png)
...
![Phase 2 ablation: honest search makes belief levels order correctly](ablation2.png)
...
![Choice-aware guessing breaks the constraint-only ceiling](guessing2.png)

![Top-1 vs top-2 accuracy: by the last trick the choice guesser's top two candidates contain the truth 99% of the time](topk.png)
...
![Strict choice-filtering degrades badly against opponents who deviate; soft filtering with slack does not](robustness.png)
...
![Value net v1: learning curves and baselines](value_train_v1.png)
...
![Phase 4 ablation: learned value vs heuristic playouts](ablation4.png)
...
![Guessing accuracy off home turf: FULL, CHOICE-strict, CHOICE-soft, PROFILER, PROFILER-ORACLE against held-out personalities](guessing5.png)

![Phase 5 headline ablation: honest-FULL vs every profiled configuration against held-out trios](ablation5.png)

## Current performance at a glance

![The search ladder: from random play (11.5 pts/hand) to the full pipeline (2.87)](search_ladder.png)

![Full-pipeline ablation row: choice-aware beliefs through honest search](ablation3.png)
...
![A1: the same oracle information delivered three ways, and the measured ESS collapse behind the original penalty](sir_oracle.png)
...
### The xinxin match (2026-08-29, Phase 7 Task F2) — beating the executable reference bot under the literature's seating design, on our variant
...
**Tier 2 (the literature's 14-way duplicate seating design — every mixed seating configuration — adapted to our variant; 1,000 deals = 14,000 games, Wilcoxon signed-rank):** our seats **6.11 [6.04, 6.18]** vs xinxin's **6.89 [6.82, 6.96]** pts/hand; paired diff **−0.78 [−0.91, −0.64]**; **Wilcoxon z = −11.26** — decisive by both our test and the literature's. Zero legality-tripwire firings across all 18,000 F2 games.
...
**Scope, honestly:** the GO-MCTS paper's tournament played full Hearts (passing + moon) and beat xinxin by 1.74 pts/hand at 25.6 s/move; our variant is no-pass/no-moon, so the two results are method-comparable but never number-comparable. Within our variant, however, the field's established executable reference bot is now measured, beaten on strength, and beaten on cost. Benchmark taxonomy, stated precisely: GO-MCTS is the strongest *published* reference (no runnable artifact exists); xinxin is the established *executable* reference; OpenSpiel's ISMCTS is a configurable generic baseline, not a frontier model. The canonical claim wording: on 1,000 randomly dealt no-pass/no-moon hands across all 14 mixed seating configurations, our 50×20 fused bot beat xinxin at its published 2,000-run/50-world/C=0.4 configuration by 0.776 points per seat-hand (paired bootstrap 95% CI [0.642, 0.910] in our favor) — this does not establish superiority in full Hearts with passing and moon scoring.
...
22. **The human-data clock has not started.** 6B (the terminal relay for logging real games) was specified and has now been built (Phase 7 Task F1, awaiting live use), and the owner's live-session gate for 6C was waived rather than met — Phase 6's entropy-curve and exploitability results are both measured entirely on synthetic self-play. The project's headline goal, beating real human players, remains untested end to end.
