...
18. **C-bench did not test the project's speed claim.** The generic OpenSpiel ISMCTS bot, not xinxin (the academic reference bot the comparison was designed for), was the opponent — xinxin needs a from-source build this phase deferred. The strength comparison (we outscored the untuned generic ISMCTS baseline at comparable per-decision cost) is real but is a claim about the value of domain knowledge over a domain-agnostic baseline, not about OpenSpiel's best; the ~20× speed-vs-xinxin claim remains unverified by any third party.
